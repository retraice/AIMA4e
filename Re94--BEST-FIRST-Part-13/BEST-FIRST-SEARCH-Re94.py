...
class PriorityQueue:
    ...
    def __init__(self, items=(), key=lambda x: x): 
        self.key = key
        print('This is PriorityQueue after self.key = key, and key=', key(2))
        self.items = [] # a heap of (score, item) pairs
        for item in items:
            print('This is PriorityQueue self.items...', self.items)
            self.add(item)
            print('This PriorityQueue self.items after self.add(item)...', self.items)
...
def best_first_search(problem, f):            
    ...
    print('First *frontier.items* PriorityQueue is...', frontier.items) 
    reached = {problem.initial: node}   # create dict. reached: {'<problem initial state>': '<Node(problem.initial)>',}
    print('First *reached* dict. of state:node pairs is...', reached) 
    while frontier:                          # Starting with first node in frontier, ??and continuing through queue??
        ...
        print('After pop, *frontier.items* is...', frontier.items)
        if problem.is_goal(node.state):      # check if the state of this node is the goal state of the problem
            print('Here is your goal!...', node, node.state, len(node), node.parent, node.action, node.path_cost)
        ...
        for child in expand(problem, node):  # if not, expand the node and for each child node of it....
            ...
            print('In for loop, *child.state* is...', s)
            if s not in reached or child.path_cost < reached[s].path_cost: # if node is new or cheaper than known ones
                ...
                print('frontier.items is...', frontier.items)
